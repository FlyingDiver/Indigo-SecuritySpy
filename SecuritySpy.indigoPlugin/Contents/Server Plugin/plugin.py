...
class Plugin(indigo.PluginBase):
    ...
    def __init__(self, pluginId, pluginDisplayName, pluginVersion, pluginPrefs):
        ...
        self.spy_servers = {}       # indigo devices for SecuritySpy servers
        self.spy_cameras = {}       # indigo devices for SecuritySpy cameras
    ...
    def camera_event(self, event_info):
        ...
        for camera_num, camera_data in event_info.items():
            ...
            self.logger.debug(f"camera_event: camera = {camera_address}, event_type = {camera_data['event_type']}, event_object = {camera_data['event_object']}")
            ...
            for triggerID in self.triggers:
                ...
                if not ((trigger.pluginProps["camera"] == "-1") or (trigger.pluginProps["camera"] == camera_address)):
                    self.logger.debug(f"{trigger.name}: Wrong camera for trigger, skipping")
                    continue
    ...
    def update_camera_device(self, camera_address, camera_info):
        ...
        self.logger.debug(f"{device.name}: Updating Camera Information")
